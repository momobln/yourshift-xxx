...
const list = await prisma.swapRequest.findMany({ include: { shift: true, fromGuard: true, toGuard: true }, orderBy: { createdAt: "desc" } });
...
const reqst = await prisma.swapRequest.create({ data: { shiftId, fromGuardId: guard.id } });
return Response.json(reqst, { status: 201 });
}